...
async function getContent(){
  const html = await fetch('http://localhost/curso/expenses/expenses/create').then(res => res.text());
  return html;
}
...
google.charts.load('current', {'packages':['bar']});
      ...
      async function drawChart() {
        const http = await fetch('http://localhost/curso/expenses/expenses/getExpensesJSON')
        .then(json => 
           json.json())
        .then(res => res);
        if(http.length == 2){
          ...
          textito.classList.add("titi");
          textito.textContent = "Necesita hacer una expense para ver los gráficos";
          elemento.appendChild(textito);
          return; 
        }
       let expenses = [...http];
        expenses.shift();
